...
<!-- Universal variant: the Claude Code-specific parts (sub-agent fan-out,
     the plugin-cache import path) are generalised here. Every rule, gate and
     checklist item is identical to SKILL.md; the two are kept in step by hand. -->
...
import it from your tool's plugin directory, the `graph.json` schema — and
...
1. **Understand the thing to draw.** For a repo, read its README and file layout
   ...
   **Large repo? Fan out (optional).** Count source files (excluding
   `node_modules/`, `.git/`, `__pycache__/`). At **≤ 80, or with no sub-agent
   support**, explore sequentially — the overhead is not worth it. Past that, if
   your tool can run read-only sub-agents in parallel, dispatch 3 (one per lens,
   never more than 5): *structure + entry-points*, *data-flow + integration* (the
   lens that must reason about flow — give it your strongest model), and
   *distribution + packaging*. Each returns **≤ 60 lines** of JSON —
   `components` (real file names, never "ServiceA"), directed `edges`
   (`{src, dst, label}` with a verb), its `group`. Merge, **dedup by id**, lay
   out, and never re-read a file a sub-agent covered.
